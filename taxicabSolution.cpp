#include <cstdio>
#include <cstring>
#include <cmath>
#include <iostream>
#include <vector>
#include <set>
#include <map>
#include <queue>
#include <stack>
#include <string>
#include <unistd.h>
#include <algorithm>
using namespace std;
#define SZ(x) ( (int) (x).size() )
#define dbg(x) cerr << #x << " = " << x << endl;
#define mp make_pair
#define pb push_back
#define fi first
#define se second
typedef long long ll;
typedef pair<int, int> pii;
// const int INF = 1e9;
const int MAX_N = 2e5 + 1;
const int POINT = -1;
const int QUERY = +1;

int N;
ll W, H;
int px[MAX_N], py[MAX_N];
set<int> E[MAX_N]; //edges
struct fenwick {
    vector<ll> t; // the mapped tree
    vector<ll> a; // it is the array of [0...n-1]
    void clear(){
        t.clear();
        a.clear();
    }
    void add(ll x){
        a.push_back(x);
    } //why we need add function? and why we need this?
    void init(){
        sort(a.begin(), a.end());
        t.assign(SZ(a) + 1, 0); // set all the number to 0
    }
    void update(ll val, ll x){
        int i = lower_bound(a.begin(), a.end(), val) - a.begin(); // use lower bounds to calcuate the index
        //using a to find the value of i
        for(++i; i < SZ(t); i += i & -i) {
            t[i] += x;
        }
    }
    ll query(ll val){
        int i = lower_bound(a.begin(), a.end(), val) - a.begin();
        ll s = 0;
        for(++i; i > 0; i -= i & -i) {
            s += t[i];
            //go to the parent.
        }
        return s;
    }
} fwt[MAX_N];


struct point {
    ll x, y; // the x
    int c, e;
    bool operator<(const point& o) const{
        if(x != o.x)
            return x < o.x;
        if(y != o.y)
            return y < o.y;
        return e < o.e;
    }
} pts[MAX_N * 2];
int npts;

// tree size
int ts[MAX_N];

void dfs(int u, int p){
    ts[u] = 1;
    for(int v: E[u])
        if(v != p) {
            dfs(v, u);
            ts[u] += ts[v];
        }
}

int center(int u){
    int p = -1, s = ts[u];
    while(true){
        int n = -1;
        for(int v: E[u]){
            if(v != p && ts[v] * 2 > s){
                n = v;
                break;
            }
        }
        if(n == -1)
            return u;
        
        p = u;
        u = n;
    }
    return u;
}

void getPoints(int u, int p, ll a, ll b, int c){
    for(int v: E[u]) {
        if(p != v){
            ll na = a + abs(px[u] - px[v]);
            ll nb = b + abs(py[u] - py[v]);
            int nc = (c == -1 ? v : c); // assign new color if not exists
            pts[npts++] = {na, nb, nc, POINT};
            pts[npts++] = {W - na, H - nb, nc, QUERY};
            getPoints(v, u, na, nb, nc);
        }
    }
}

ll geometry(int u){
    ll ret = 0;
    // clear fenwick trees
    fwt[u].clear();
    for(int v: E[u]) fwt[v].clear();
    
    // First get all points
    npts = 0;
    getPoints(u, -1, 0, 0, -1);
    // And add them to our fenwick trees ...
    for(int i = 0; i < npts; i++){
        fwt[pts[i].c].add(pts[i].y);
        fwt[u].add(pts[i].y);
    }
    fwt[u].init();
    for(int v: E[u]) fwt[v].init();
    
    // Now sweep line
    sort(pts, pts + npts);
    for(int i = 0; i < npts; i++){
        if(pts[i].e == POINT){
            fwt[u].update(pts[i].y, 1);
            fwt[pts[i].c].update(pts[i].y, -1);
        } else {
            ret += fwt[u].query(pts[i].y) + fwt[pts[i].c].query(pts[i].y);
        }
    }
    // We counted too much
    ret /= 2;
    
    // Now add the paths between root and the nodes.
    for(int i = 0; i < npts; i++) {
        ret += (pts[i].x <= W && pts[i].y <= H && pts[i].e == POINT);
    }
    return ret;
}

ll dq(int u){
    dfs(u, -1);
    u = center(u);
    ll ans = geometry(u);
    for(int v: E[u]) {
        E[v].erase(u);
        ans += dq(v);
    }
    return ans;
}

ll solve(){
    return 1LL * N * (N - 1) / 2 - dq(1);
}

int main(){
    FILE * pfile;
    int u, v;
    char buf[200];
    printf("%s\n", getcwd(buf, 200));
    pfile = fopen("/Users/yunxiaozou/taxicabDriver2.txt", "r");
    fscanf(pfile, "%d%lld%lld", &N, &W, &H);
    
    //save the points
    for(int i = 1; i <= N; i++){
        fscanf(pfile, "%d%d", px + i, py + i);
    }
    for(int i = 0; i < N - 1; i++){
        fscanf(pfile, "%d%d", &u, &v);
        // insert edges.
        E[u].insert(v);
        E[v].insert(u);
    }
    printf("%lld\n", solve());
    return 0;
}
